...
public class SiteService {
    ...
    public SiteResponse detail(String slug, int rawTake) {
        int take = Math.min(Math.max(rawTake, 50), 1000);
        ...
        List<SiteResponse.Measurement> rows = jdbcTemplate.query("""
            SELECT `index`, date, hepres, heleve, actemp, achumi FROM mrtb
            ...
            """, (rs, row) -> new SiteResponse.Measurement(rs.getInt("index"),
                ...
                DashboardService.parseNumber(rs.getString("achumi"))), site.siteDb(), take);
